...
| **Phase 2 plan** | `docs/phase-2-plan.md` — approved; §8 has a live Progress table. First build pass complete: all pages + schema + RLS + Edge Function written; local gate green. |
| **Blocked-on for full Phase 2 completion** | (a) `SUPABASE_ACCESS_TOKEN` or `npx supabase login` to apply migrations + `db push` the seed + deploy `submit`. (b) Cloudflare Turnstile keys + Resend key — for spam protection + a real soft-launch. (c) Remaining §14.2 content (final service copy, About detail, real testimonials, Privacy Policy legal review, booking lead-time/windows). (d) WP11 Lighthouse/axe + WP14 Vercel staging (separate gate). |
...
| 2026-09-07 | **Phase 2 build — first pass.** Written locally (all green: typecheck/lint/format/test(12)/build): `supabase/` migrations (core schema, RLS + public views, storage bucket), `supabase/seed.sql` (confirmed facts + DRAFT copy), `supabase/functions/submit/` Edge Function (honeypot+timing+optional Turnstile+validate+insert+optional Resend). Front end: `src/shared/{seo,analytics}.ts`, `src/shared/content/{queries,media}.ts` (graceful degradation), `src/shared/supabase/types.ts`; UI primitives (Container/Section/Button/Field/Prose/SeoHead/Img/CallWhatsApp); chrome (Nav/Footer/MobileContactBar/CfAnalytics); **all 14 public routes** with loaders + getStaticPaths + forms; `scripts/gen-sitemap.mjs`; `robots.txt`. Added `marked`. Build prerenders 12 pages + sitemap; `/admin` excluded/code-split. **Not yet done:** migrations/function NOT applied/deployed (needs Supabase CLI auth); Lighthouse/axe pass; Turnstile/Resend keys; RLS test harness (needs live schema); WP14 Vercel staging. | Lead architect (TECHBIGGIEY) |
