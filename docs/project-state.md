...
| **Phase 3** | **IN PROGRESS. WP1–WP5 + WP3b done** (`docs/phase-3-plan.md` §6): auth (email+password, owner account live), `profile`/`client`/`vehicle`/`notification` schema + `authenticated` RLS (verified live), admin shell with **Rev 01 branding** + password toggle, Dashboard, Notifications, Requests (filter/detail/status/**convert-to-client**), Schedule (confirm/decline), Team (owner-only, `admin-invite` function). **Left:** WP6 Clients/Vehicles, WP7–9 CMS+Settings, WP10 publish→rebuild (needs Vercel Deploy Hook), WP11 customer email (needs Resend key), WP12–14. |
