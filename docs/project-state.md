...
| **Phase 3** | **IN PROGRESS. WP1–WP3 done** (`docs/phase-3-plan.md` §6): auth (email+password) + `profile`/roles + guard + login/forgot/reset; `client`/`vehicle`/`notification` schema + `authenticated` full-CRUD RLS (applied live; anon sealed); admin shell (nav, unread badge, View-site link, TanStack Query). Dedicated admin entry **deferred** — kept the `/admin/*` lazy chunk. **Blocked on owner:** create the admin account (Supabase dashboard) to finish auth E2E; Vercel Deploy Hook (WP10); Resend key (WP11); staff email (WP3b). WP4–WP14 = placeholder routes. |
