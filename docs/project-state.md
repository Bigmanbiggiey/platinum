...
> **Last updated:** 2026-09-08 — Phase 3 functionally complete (WP1–WP13); Phase 2 still on staging.
...
| **Phase 3** | **FUNCTIONALLY COMPLETE (WP1–WP13)** — `docs/phase-3-plan.md` §6. Auth + roles + Team; Dashboard + Notifications; Requests (filter/detail/status/**convert-to-client**) + Schedule (confirm/decline + customer email); **full CMS** (Services, Portfolio + gallery, Testimonials moderation, Media library, Page copy, Areas, Partners); **Business Settings**. publish→rebuild + customer/owner email functions deployed but **no-op until the owner sets `VERCEL_DEPLOY_HOOK_URL` / `RESEND_API_KEY` Supabase secrets**. Rev 01 branding across the admin; password toggle; `<Prose>` sanitised. 24 tests. **WP14:** owner to do the real end-to-end acceptance run. |
...
| 2026-09-08 | **Phase 3 WP6–WP13 + landing CTA.** WP9 Business Settings editor (NAP, hours, SEO, notification channel/address, socials); Content hub + Page copy (Markdown+preview) + Areas + Partners (generic `InlineCrud`). WP7 Services/Portfolio list+edit (Markdown+preview, publish, SEO, cover + gallery) + Testimonials moderation. WP8 Media library (upload to `public-media`, alt text, delete). WP6 Clients+Vehicles (list/search/detail/nested vehicles/linked requests). WP10 `rebuild` Edge Function + `usePublish()` (debounced; no-op without `VERCEL_DEPLOY_HOOK_URL`). WP11 `notify-customer` Edge Function wired into Schedule confirm/decline (no-op without `RESEND_API_KEY`). WP12 `docs/owner-guide.md`. WP13 admin tests → **24 pass**. `<Prose>` sanitises (DOMPurify/regex); added `dompurify`. Home CTA rewritten per owner ("Expert automotive engineering, wherever you need it." / "Need help with your vehicle? Let's get it assessed.") → DB content blocks + `seed.sql` + hero shows Call/WhatsApp/Request a Service. Deployed functions `rebuild` + `notify-customer`. Gate green (typecheck/lint/test 24/build; admin chunk 143 kB gzip). Pushed → auto-deployed. | Lead architect (TECHBIGGIEY) |
| 2026-09-07 | **Phase 3 WP3b + WP4 + WP5 + admin branding.** Rev 01 datum mark/wordmark across AdminShell/Login/Reset; admin UI kit (`--color` tokens, IBM Plex Mono for data); **password show/hide toggle** on login + reset. WP4: Dashboard (clickable counts + recent requests) + Notifications page (list/mark-read/click-through) + unread badge. WP5: Requests table (filters) + detail (status pipeline, notes, **convert to client + vehicle**) + Schedule (Confirm/re-time/Decline; customer email deferred to WP11). WP3b: `admin-invite` Edge Function (owner-checked; service-role create-user + one-time invite link) + `profile.email` (migration `20260907110000` + trigger + backfill) + Team page. TanStack Query hooks in `src/admin/lib/*`. Applied migration; deployed `admin-invite`. Gate green (typecheck/lint/test 19/build; admin chunk 112 kB gzip). | Lead architect (TECHBIGGIEY) |
| 2026-09-07 | **Phase 3 WP1–WP3.** Applied migrations `20260907100000_admin_schema` (`profile`+role+`handle_new_user`, `is_admin/is_owner`, `client`, `vehicle`, `notification`+triggers, service_request FKs, `created_by`), `..._100_admin_rls` (`authenticated`+`is_admin()` full-CRUD on all tables + Storage; profile self/owner), `..._200_seal_admin_tables` (revoke anon → 401). `seed.sql` promotes `gatama98p@gmail.com` to `owner`. Front end: `shared/supabase/auth.ts`, `AuthProvider`/`authContext`, real `RequireAuth` guard, `AdminShell` (nav + unread badge + View-site link), `AdminApp` (TanStack Query + routes), login/forgot/reset + Dashboard (live counts) + placeholders; removed Phase 1 stubs; added `@tanstack/react-query`. Verified anon still 401 on private tables; `/admin`→`/admin/login`; typecheck/lint/test(18)/build green. **Owner must create the admin account (Supabase dashboard) to finish auth + the authed RLS test.** | Owner + Lead architect (TECHBIGGIEY) |
